...
export default function Musicas() {
  ...
  const [formData, setFormData] = useState({ titulo: '', artista: '', tom_original: '', bpm: '' });
  ...
  const handleDelete = (id) => {
    const confirmacao = window.confirm("Tem certeza que deseja deletar esta música?");
    ...
    <div>
      <div className="lauda-page-header">
        <div>
          ...
          <p className="text-muted">Gerencie o repertório global do ministério</p>
        </div>
        <button className="lauda-btn lauda-btn-primary" onClick={handleNovoClick}>
          + Nova Música
        </button>
      </div>
        ...
        {musicas.map(musica => (
          <div key={musica.id} className="lauda-card">
            ...
            <div className="musica-meta text-muted">
              <span><strong>Tom:</strong> {musica.tom_original}</span>
              {musica.bpm && <span><strong>BPM:</strong> {musica.bpm}</span>}
            </div>
            <div style={{ marginTop: '15px', display: 'flex', gap: '10px', justifyContent: 'flex-end', borderTop: '1px solid var(--gray-200)', paddingTop: '15px' }}>
              <button className="lauda-btn lauda-btn-secondary" style={{ padding: '5px 10px', fontSize: '0.85rem' }} onClick={() => handleEditarClick(musica)}>
                Editar
              </button>
              <button className="lauda-btn lauda-btn-danger" style={{ padding: '5px 10px', fontSize: '0.85rem', backgroundColor: 'var(--error-dark)', color: 'white', border: 'none' }} onClick={() => handleDelete(musica.id)}>
                Deletar
              </button>
            </div>
          </div>
        ...
        <div className="modal-overlay">
          ...
          <div className="modal">
            <div className="modal-header">
              <h3 className="modal-title">{editingId ? 'Editar Música' : 'Cadastrar Nova Música'}</h3>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="modal-body">
                ...
                <div className="input-group">
                  <label className="input-label">Artista/Banda Original *</label>
                  <input
                    type="text" name="artista" className="input-field"
                    value={formData.artista} onChange={handleChange} required
                  />
                </div>
                ...
                <div className="form-row">
                  <div className="input-group">
                    ...
                    <input
                      type="text" name="tom_original" className="input-field"
                      placeholder="Ex: C, Dm, G#"
                      value={formData.tom_original} onChange={handleChange} required
                    />
                  </div>
                  <div className="input-group">
                    <label className="input-label">BPM</label>
                    <input
                      type="number" name="bpm" className="input-field"
                      placeholder="Ex: 120"
                      value={formData.bpm} onChange={handleChange}
                    />
                  </div>
                </div>
